    ...
    private optimizeBlock(block: BasicBlock): void {
        if (block.terminator.kind === "conditional") {
            ...
            if (sameStatements !== -1) {
                ...
                trueBranch.stmts = trueBranch.stmts.slice(
                    0,
                    trueBranch.stmts.length - sameStatements,
                );
                falseBranch.stmts = falseBranch.stmts.slice(
                    0,
                    falseBranch.stmts.length - sameStatements,
                );
            }
        }
    }
